...
export function LoginSignup() {
  ...
  function checkGoogleCredentials(credentials) {
    const user = users.find(
      (currUser) =>
        currUser.fullname === credentials.name &&
        currUser.username === credentials.email
    );
    if (user) login(user);
    else {
      signup({
  ...
  return (
    ...
    <div className="login-signup">
      <LoginPageHeader />
      <form
        className="form-container layout"
        onSubmit={(ev) => onSubmit(ev, isSignup)}>
        <h1>
          {isSignup
            ? "Create your MyDay account here "
            : "Log in to your account"}
        </h1>
        {isSignup && <ImgUploader onUploaded={onUploaded} />}
        {!isSignup && (
          <p className="login-explain">Enter your username and password</p>
        )}
        {isSignup && (
        ...
        <div className="suggest-signup">
          <span className="suggest-signup-prefix">
            {isSignup
